...
- Versioned `native_v3_student_v1` in Contract as the only shared authority
  for architecture-neutral corridor and exemplar batch materialization.
- Bound the frozen v3 `training_payload.logical_id` values to stable Contract
  roles through an explicit content-manifest mapping without changing v3
  identity semantics.
- Path A and Path B are provenance only; transport and delivery route cannot
  change target, alignment, mask, loss, or weight semantics.
- Student must call Contract's public validator and must not import Tome or
  infer physical filenames, array members, axes, or joins.
