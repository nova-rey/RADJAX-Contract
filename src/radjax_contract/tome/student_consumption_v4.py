"""Native-v3 Student-consumption v4 admission.

V4 keeps delivery and score-authority receipts integrity-bound and semantically
validated, but deliberately excludes their body digests from batch-semantic
identity. Equivalent Path A and Path B payloads therefore retain one
consumption identity while preserving distinct auditable provenance.
"""
...
_MANIFEST_PATH = "manifests/student_consumption_v4.json"
_IDENTITY_SCHEMA = "radjax_tome_student_consumption_semantic_identity_v4"
_PROVENANCE_ONLY_ROLES = frozenset({"delivery_receipt", "authority_reference"})


def contract_root() -> Path:
    """Return the installed v4 Student-consumption contract assets."""
...
class StudentConsumptionV4Descriptor:
    schema_version: str
    contract_id: str
    profile_id: str
...
@dataclass(frozen=True)
class StudentConsumptionV4ValidationResult:
    ok: bool
    profile_id: str
    issues: tuple[_v1.StudentConsumptionIssue, ...]
    warnings: tuple[_v1.StudentConsumptionIssue, ...]
    descriptor: StudentConsumptionV4Descriptor | None

    def to_dict(self) -> dict[str, Any]:
        return {
            "schema_version": "radjax_student_consumption_validation_result_v4",
            "ok": self.ok,
...
def validate_and_resolve_student_consumption_v4(
    artifact: str | Path, *, strict: bool = False
) -> StudentConsumptionV4ValidationResult:
    """Validate an explicitly negotiated v4 profile; never downgrade it."""
    ...
    with _v1._artifact_root(artifact_path, issues, warnings) as root:
        ...
        if cover is None:
            return _result(issues, warnings)
        if cover.get("schema_version") != "radjax_tome_cover_v3_student_consumption_v4":
        ...
        if (
            ...
            != "radjax_tome_student_consumption_manifest_v4"
        ):
            issues.append(_issue("TSC001_PROFILE_UNSUPPORTED", "profile_cover"))
            return _result(issues, warnings)
        identity = manifest.get("semantic_identity")
        if not isinstance(identity, dict) or not _identity_matches(
            identity, sidecar, manifest
        ):
            issues.append(
        ...
        _validate_v4_evidence(root, manifest, identity, descriptor, issues)
        warnings.extend(_reprofile_copy(v2_result.warnings))
        ...
        return _result(
            ...
            StudentConsumptionV4Descriptor(
                "radjax_student_consumption_descriptor_v4",
                CONTRACT_ID,
                PROFILE_ID,
...
@contextmanager
def open_verified_student_resource_v4(
    artifact: str | Path, resource_id: str, *, strict: bool = False
):
    """Open one v4-admitted resource while rechecking its raw integrity."""

    result = validate_and_resolve_student_consumption_v4(artifact, strict=strict)
...
def _resolve_base_v2(
    ...
    with tempfile.TemporaryDirectory(prefix="radjax-tsc-v4-") as tmp:
        ...
        # schema intentionally has no v4 score-authority extension.
        identity["authority"] = {
        ...
        # V2 validates the complete resource inventory in this private staging
        # directory. V4 excludes provenance only from its public identity.
        identity["resources"] = [
            {
                key: row[key]
        ...
        identity["semantic_digest"] = (
            "sha256:"
            + hashlib.sha256(
                json.dumps(v2_projection, sort_keys=True, separators=(",", ":")).encode(
                    "utf-8"
                )
            ).hexdigest()
        )
        ...
        # transport declaration must describe that directory, while v4 has
...
def _identity_projection(identity: dict[str, Any]) -> dict[str, Any]:
    """Return the exact v4 batch-semantic projection.

    Provenance receipt bodies remain manifest-bound, raw-integrity checked, and
    cross-validated. They are not batch inputs and must not distinguish
    equivalent Path A and Path B consumption semantics.
    """
    ...
    if isinstance(resources, list):
        projection["resources"] = [
            ...
            and resource.get("role") not in _PROVENANCE_ONLY_ROLES
        ]
    return projection
...
def _identity_matches(
    identity: dict[str, Any], sidecar: dict[str, Any], manifest: dict[str, Any]
) -> bool:
    expected_resources = [
        {
            key: resource[key]
    ...
    return (
        ...
        and identity.get("resources") == expected_resources
...
def _validate_manifest(manifest: dict[str, Any]) -> bool:
    return _validate_schema(
        manifest,
        "student_consumption_manifest_v4.json",
        "student_consumption_semantic_identity_v4.json",
    )


def _validate_cover(cover: dict[str, Any]) -> bool:
    return _validate_schema(cover, "tome_cover_v3_student_consumption_v4.json")
...
def _result(
    issues: list[_v1.StudentConsumptionIssue],
    warnings: list[_v1.StudentConsumptionIssue],
    descriptor: StudentConsumptionV4Descriptor | None = None,
) -> StudentConsumptionV4ValidationResult:
    ...
    return StudentConsumptionV4ValidationResult(
        not ordered,
        PROFILE_ID,
        ordered,
        ordered_warnings,
