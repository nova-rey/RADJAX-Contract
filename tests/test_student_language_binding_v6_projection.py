...
def test_v6_projection_is_public_in_built_wheel(tmp_path: Path) -> None:
    ...
    subprocess.run(
        [
            ...
            "assert callable(open_verified_student_multipart_resource_v6); "
            "assert callable(open_verified_student_resource_component_v6); "
            "assert VerifiedStudentMultipartComponentV6.__module__.startswith("
            "'radjax_contract'); "
            "assert VerifiedStudentMultipartResourceV6.__module__.startswith("
            "'radjax_contract'); "
